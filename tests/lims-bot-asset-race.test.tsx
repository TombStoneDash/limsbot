...
describe("asset generation races", () => {
  ...
  it.each(outcomes)("keeps the replacement loading when the old request finishes first with %s", async (outcome) => {
    ...
    expect(button("Drafting…").disabled).toBe(true);
    expect(container.querySelectorAll("textarea")).toHaveLength(1);
    await settle(replacement, "success", "Bench centrifuge maintenance");
    expect(button("Generate draft").disabled).toBe(false);
    expect(container.querySelectorAll("textarea")[1].value).toBe(draft("Bench centrifuge maintenance").draftRecord);
    await click("Approve");
    const events = container.querySelectorAll("aside li");
    const event = Array.from(events).find((node) => node.textContent?.includes("Bench centrifuge maintenance"));
  ...
  it.each([
  ...
  ] as const)("ignores obsolete JSON parsing that later %ss (replacement completed=%s)", async (outcome, completed) => {
    ...
    const replacement = queueRequest();
    await click("Generate draft");
    if (completed) await settle(replacement, "success", "Bench centrifuge maintenance");
    await act(async () => {
    ...
    expect(container.querySelectorAll("textarea")[1].value).toBe(draft("Bench centrifuge maintenance").draftRecord);
    expect(button("Generate draft").disabled).toBe(false);
    expect(container.textContent).toContain("Bench centrifuge maintenance");
  });
  ...
  it("clears a completed edited draft and scan while preserving notes and audit entries", async () => {
    const approved = queueRequest();
    await click("Generate draft");
    await settle(approved, "success", "Previously approved Sciclone record");
    await click("Approve");
    ...
    await click("Generate draft");
    await settle(old, "success");
    await edit(container.querySelectorAll("textarea")[1], "Old edited Sciclone record");
    await click("Bench centrifuge");
    expectNoObsoleteState();
    expect(container.querySelectorAll("textarea")).toHaveLength(1);
    expect(container.querySelector("textarea")!.value).toBe("Keep this operator note");
    expect(container.textContent).not.toContain("Old edited Sciclone record");
    expect(container.textContent).not.toContain("✓ Scanned");
    ...
    expect(JSON.parse(fetchMock.mock.calls[2][1].body)).toMatchObject({
      userMessage: "Keep this operator note",
      context: { asset: "Bench centrifuge" },
    });
    await settle(replacement, "success", "Bench centrifuge maintenance");
    expect(container.querySelectorAll("textarea")[1].value).toBe(draft("Bench centrifuge maintenance").draftRecord);
    await click("Approve");
    expect(container.querySelector("aside")!.textContent).toContain("Previously approved Sciclone record");
    expect(container.querySelector("aside")!.textContent).toContain("Bench centrifuge maintenance");
    expect(container.querySelector("aside")!.textContent).not.toContain("edited-approved");
  });

  it("preserves pending requests, scans, notes, and draft edits when selecting the same asset", async () => {
    await click("Mock scan");
    await edit(container.querySelector("textarea")!, "Keep this note");
    const pending = queueRequest();
    await click("Generate draft");
    await click("Sciclone G3 NGSx Workstation");
    expect(button("Drafting…").disabled).toBe(true);
    expect(container.textContent).toContain("✓ Scanned");
    await settle(pending, "success", "Current Sciclone maintenance");
    await edit(container.querySelectorAll("textarea")[1], "Edited current Sciclone record");
    await click("Sciclone G3 NGSx Workstation");
    expect(container.querySelectorAll("textarea")[1].value).toBe("Edited current Sciclone record");
    expect(container.querySelector("textarea")!.value).toBe("Keep this note");
    expect(container.textContent).toContain("✓ Scanned");
    expect(button("Generate draft").disabled).toBe(false);
    await click("Approve");
    expect(container.querySelector("aside")!.textContent).toContain("Current Sciclone maintenance");
