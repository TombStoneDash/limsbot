...
describe("asset generation races", () => {
  ...
  it("invalidates a completed draft and resets the scan while preserving operator notes", async () => {
    await click("Mock scan");
    await edit(container.querySelector("textarea")!, "Keep this operator note");
    const old = queueRequest();
    await click("Generate draft");
    await settle(old, "success");
    await edit(container.querySelectorAll("textarea")[1], "Edited Sciclone record");
    await click("Bench centrifuge");
    expectNoObsoleteState();
    expect(container.querySelectorAll("textarea")).toHaveLength(1);
  ...
  it("preserves approved audit entries and operator notes across asset changes", async () => {
    const approved = queueRequest();
    await click("Generate draft");
    await settle(approved, "success", "Approved Sciclone maintenance");
    await click("Approve");
    const audit = container.querySelector("aside")!.textContent;
    await edit(container.querySelector("textarea")!, "Next maintenance note");
    await click("Bench centrifuge");
    expect(container.querySelector("aside")!.textContent).toBe(audit);
    expect(audit).toContain("Approved Sciclone maintenance");
    expect(audit).toContain("instrument_maintenance · approved · template");
    expect(container.querySelector("textarea")!.value).toBe("Next maintenance note");
  });

  it("preserves the pending request, scan, notes and draft edits when reselecting the same asset", async () => {
    await click("Mock scan");
    await edit(container.querySelector("textarea")!, "Keep this operator note");
    ...
    await click("Sciclone G3 NGSx Workstation");
    expect(container.textContent).toContain("Current Sciclone draft");
  ...
  it("preserves an existing error when reselecting the same asset", async () => {
    const failed = queueRequest();
    await click("Generate draft");
    await settle(failed, "http");
    await click("Sciclone G3 NGSx Workstation");
