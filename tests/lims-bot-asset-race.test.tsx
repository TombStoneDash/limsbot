...
async function settle(request: ReturnType<typeof queueRequest>, outcome: Outcome, title = "Obsolete Sciclone draft") {
  await act(async () => {
...
function expectNoObsoleteState() {
  expect(container.textContent).not.toContain("Obsolete Sciclone draft");
...
describe("asset generation races", () => {
  ...
  it.each(outcomes)("keeps the replacement loading when the old request finishes first with %s", async (outcome) => {
    ...
    await click("Generate draft");
    expect(JSON.parse(fetchMock.mock.calls[1][1].body).context.asset).toBe("Bench centrifuge");
    await settle(old, outcome);
    expectNoObsoleteState();
    expect(button("Drafting…").disabled).toBe(true);
    expect(container.querySelectorAll("textarea")).toHaveLength(1);
    await settle(replacement, "success", "Fresh centrifuge maintenance");
    expect(button("Generate draft").disabled).toBe(false);
    expect(container.querySelectorAll("textarea")[1].value).toBe(draft("Fresh centrifuge maintenance").draftRecord);
    await click("Approve");
    const events = container.querySelectorAll("aside li");
    const event = Array.from(events).find((node) => node.textContent?.includes("Fresh centrifuge maintenance"));
  ...
  it.each(["resolve", "reject"] as const)("ignores obsolete JSON parsing that later %ss", async (outcome) => {
    ...
    await click("Generate draft");
    await act(async () => {
      if (outcome === "resolve") body.resolve(draft("Obsolete Sciclone draft"));
      else body.reject(new Error("Invalid JSON"));
    });
    expectNoObsoleteState();
    expect(button("Drafting…").disabled).toBe(true);
    await settle(replacement, "success", "Fresh centrifuge maintenance");
    expect(container.textContent).toContain("Fresh centrifuge maintenance");
  });
...
describe("asset selection state", () => {
  it("clears completed drafts, edits and scan state while preserving notes and audit history", async () => {
    const approved = queueRequest();
    await click("Generate draft");
    await settle(approved, "success", "Prior approved maintenance");
    await click("Approve");
    const auditBefore = container.querySelector("aside")!.textContent;
    await edit(container.querySelector("textarea")!, "Keep operator notes");
    await click("Mock scan");
    const old = queueRequest();
    await click("Generate draft");
    await settle(old, "success");
    await edit(container.querySelectorAll("textarea")[1], "Obsolete edited record");
    await click("Bench centrifuge");
    expectNoObsoleteState();
    expect(container.querySelectorAll("textarea")).toHaveLength(1);
    expect(container.querySelector("textarea")!.value).toBe("Keep operator notes");
    ...
    expect(container.querySelector("aside")!.textContent).toBe(auditBefore);
    await click("Sciclone G3 NGSx Workstation");
    expect(container.querySelectorAll("textarea")).toHaveLength(1);
    await click("Bench centrifuge");
    const fresh = queueRequest();
    await click("Generate draft");
    expect(JSON.parse(fetchMock.mock.calls[2][1].body)).toMatchObject({
      workflow: "instrument_maintenance", userMessage: "Keep operator notes",
      context: { asset: "Bench centrifuge", assetType: "Sample prep" },
    });
    await settle(fresh, "success", "Fresh centrifuge maintenance");
    expect(container.querySelectorAll("textarea")[1].value).toBe(draft("Fresh centrifuge maintenance").draftRecord);
    await click("Approve");
    const events = container.querySelectorAll("aside li");
    expect(events[0].textContent).toContain("Fresh centrifuge maintenance");
    expect(events[0].textContent).toContain("instrument_maintenance · approved · template");
    expect(events[1].textContent).toContain("Prior approved maintenance");
  });

  it("preserves pending work, notes and scan state when selecting the same asset", async () => {
    await edit(container.querySelector("textarea")!, "Keep operator notes");
    await click("Mock scan");
    ...
    expect(container.querySelector("textarea")!.value).toBe("Keep operator notes");
    await settle(pending, "success", "Current Sciclone draft");
    expect(container.querySelectorAll("textarea")[1].value).toBe(draft("Current Sciclone draft").draftRecord);
    await edit(container.querySelectorAll("textarea")[1], "Preserved edits");
    await click("Sciclone G3 NGSx Workstation");
    expect(container.querySelectorAll("textarea")[1].value).toBe("Preserved edits");
    expect(container.textContent).toContain("✓ Scanned");
    expect(button("Approve").disabled).toBe(false);
    expect(fetchMock).toHaveBeenCalledTimes(1);
  });

  it("preserves an error on same-asset selection and clears it on a different asset", async () => {
    const failed = queueRequest();
    await click("Mock scan");
    await click("Generate draft");
    await settle(failed, "http");
