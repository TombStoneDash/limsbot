...
describe("asset generation races", () => {
  ...
  it.each(outcomes)("keeps the replacement loading when the old request finishes first with %s", async (outcome) => {
    ...
    await click("Generate draft");
    expect(JSON.parse(fetchMock.mock.calls[1][1].body).context).toMatchObject({
      asset: "Bench centrifuge", assetType: "Sample prep",
    });
    await settle(old, outcome);
    expectNoObsoleteState();
    expect(button("Drafting…").disabled).toBe(true);
    expect(container.querySelectorAll("textarea")).toHaveLength(1);
    await settle(replacement, "success", "Valid centrifuge draft");
    expect(button("Generate draft").disabled).toBe(false);
    expect(container.querySelectorAll("textarea")[1].value).toBe(draft("Valid centrifuge draft").draftRecord);
    await click("Approve");
    const events = container.querySelectorAll("aside li");
    const event = Array.from(events).find((node) => node.textContent?.includes("Valid centrifuge draft"));
    expect(event?.textContent).toContain("field_sample · approved · template");
  ...
  it.each(["resolve", "reject"] as const)("ignores obsolete JSON parsing that later %ss", async (outcome) => {
    ...
    expect(button("Drafting…").disabled).toBe(true);
    await settle(replacement, "success", "Valid centrifuge draft");
    expect(container.textContent).toContain("Valid centrifuge draft");
  });
  ...
  it("clears a loaded preview, edits, and scan while preserving audit and operator note", async () => {
    const approved = queueRequest();
    await click("Generate draft");
    await settle(approved, "success", "Previously approved record");
    await click("Approve");
    ...
    expect(Array.from(container.querySelectorAll("button"))
      .some((node) => node.textContent?.includes("Approve"))).toBe(false);
    const replacement = queueRequest();
    await click("Generate draft");
    expect(JSON.parse(fetchMock.mock.calls[2][1].body)).toMatchObject({
      userMessage: "Keep this operator note", context: { asset: "Bench centrifuge" },
    });
    await settle(replacement, "success", "Valid centrifuge draft");
    expect(container.querySelectorAll("textarea")[1].value).toBe(draft("Valid centrifuge draft").draftRecord);
    await click("Approve");
    expect(container.querySelector("aside ul")!.querySelectorAll("li")).toHaveLength(2);
    expect(container.querySelector("aside")!.textContent).toContain("Previously approved record");
    expect(container.querySelector("aside")!.textContent).toContain("Valid centrifuge draft");
  });

  it("clears scan confirmation and resets onboarding during an asset request", async () => {
    await click("Mock scan");
    const old = queueRequest();
    await click("Generate draft");
    await click("Bench centrifuge");
  ...
  it("leaves a pending request and edited preview intact when selecting the same asset", async () => {
    await edit(container.querySelector("textarea")!, "Keep note");
    await click("Mock scan");
    ...
    await settle(current, "success", "Current workstation draft");
    await edit(container.querySelectorAll("textarea")[1], "Keep edited record");
    await click("Sciclone G3 NGSx Workstation");
    expect(container.textContent).toContain("Current workstation draft");
    expect(container.querySelectorAll("textarea")[1].value).toBe("Keep edited record");
    expect(container.querySelector("textarea")!.value).toBe("Keep note");
    expect(container.textContent).toContain("✓ Scanned");
    expect(fetchMock).toHaveBeenCalledTimes(1);
  });

  it("preserves an error when selecting the same asset", async () => {
    const failed = queueRequest();
    await click("Generate draft");
    await settle(failed, "http");
    await click("Sciclone G3 NGSx Workstation");
