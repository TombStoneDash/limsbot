...
export default async function FlipperDashboardPage() {
  ...
  const limsBotDraft = `Asset ${latest.asset_id} (${latest.asset_name}) scanned at ${latest.location}. Scan captured at: ${new Date(
    latest.captured_at
  ).toUTCString()}. Calibration status is unknown: this registry does not provide calibration status; operator verification is required. Suggested next workflow step: schedule operator verification, log scan event in maintenance ledger, attach to today's run sheet. Awaiting human approval before any record is written.`;
